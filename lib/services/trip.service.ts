...
export class TripService {
  ...
  static async cloneTrip(shareCode: string, targetUserId: number) {
    ...
    const clonedTrip = await this.createTrip({
      ...
      startDate: new Date(sourceTrip.start_date).toISOString().split('T')[0],
      endDate: new Date(sourceTrip.end_date).toISOString().split('T')[0],
    ...
    for (const stop of sourceTrip.stops) {
      const newStop = await this.addStop({
        ...
        cityId: stop.city_id,
        arrivalDate: new Date(stop.arrival_date).toISOString().split('T')[0],
        departureDate: new Date(stop.departure_date).toISOString().split('T')[0],
      ...
      for (const act of stop.activities) {
        await this.addActivity({
          ...
          activityDate: new Date(act.activity_date).toISOString().split('T')[0],

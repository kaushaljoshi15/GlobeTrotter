...
export async function sendOTP(toEmail: string, code: string) {
  const mailOptions = {
    from: `"GlobeTrotter Security" <${process.env.EMAIL_USER}>`,
    to: toEmail,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #eaeaea; border-radius: 8px; padding: 24px; color: #333;">
        <h2 style="color: #2563eb; text-align: center; margin-bottom: 24px;">Welcome to GlobeTrotter!</h2>
        <p>Hello,</p>
        ...
        <p style="font-size: 12px; color: #64748b; text-align: center;">&copy; ${new Date().getFullYear()} GlobeTrotter. All rights reserved.</p>
      </div>
    `,
  };
